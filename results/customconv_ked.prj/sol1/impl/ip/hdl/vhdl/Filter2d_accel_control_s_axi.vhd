...
entity Filter2d_accel_control_s_axi is
generic (
...
end entity Filter2d_accel_control_s_axi;
...
architecture behave of Filter2d_accel_control_s_axi is
    ...
    constant ADDR_SHIFT_DATA_0 : INTEGER := 16#10#;
    constant ADDR_SHIFT_CTRL   : INTEGER := 16#14#;
    constant ADDR_ROWS_DATA_0  : INTEGER := 16#18#;
    constant ADDR_ROWS_CTRL    : INTEGER := 16#1c#;
    constant ADDR_COLS_DATA_0  : INTEGER := 16#20#;
    constant ADDR_COLS_CTRL    : INTEGER := 16#24#;
    ...
    signal int_shift           : UNSIGNED(7 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SHIFT_DATA_0 =>
                        rdata_data <= RESIZE(int_shift(7 downto 0), 32);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SHIFT_DATA_0) then
                    int_shift(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_shift(7 downto 0));
                end if;
            end if;
        end if;
    end process;

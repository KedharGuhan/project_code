...
typedef struct {
    ...
    XFilter2d_accel_uio_map maps[ MAX_UIO_MAPS ];
} XFilter2d_accel_uio_info;

/***************** Variable Definitions **************************************/
static XFilter2d_accel_uio_info uio_info;
...
static int uio_info_read_name(XFilter2d_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFilter2d_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFilter2d_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFilter2d_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFilter2d_accel_Initialize(XFilter2d_accel *InstancePtr, const char* InstanceName) {
	XFilter2d_accel_uio_info *InfoPtr = &uio_info;

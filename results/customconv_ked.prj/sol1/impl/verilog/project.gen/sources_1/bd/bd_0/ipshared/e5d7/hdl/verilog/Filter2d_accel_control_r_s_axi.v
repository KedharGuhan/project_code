...
// ==============================================================
`timescale 1ns/1ps
module Filter2d_accel_control_r_s_axi
#(parameter
    ...
    output wire [63:0]                   conv_out,
    output wire [63:0]                   maxpool_out
);
//------------------------Address Info-------------------
...
localparam
    ...
    ADDR_FILTER_DATA_0      = 6'h1c,
    ADDR_FILTER_DATA_1      = 6'h20,
    ADDR_FILTER_CTRL        = 6'h24,
    ADDR_CONV_OUT_DATA_0    = 6'h28,
    ADDR_CONV_OUT_DATA_1    = 6'h2c,
    ADDR_CONV_OUT_CTRL      = 6'h30,
    ADDR_MAXPOOL_OUT_DATA_0 = 6'h34,
    ADDR_MAXPOOL_OUT_DATA_1 = 6'h38,
    ADDR_MAXPOOL_OUT_CTRL   = 6'h3c,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_conv_out = 'b0;
    reg  [63:0]                   int_maxpool_out = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CONV_OUT_DATA_0: begin
                    rdata <= int_conv_out[31:0];
                end
                ADDR_CONV_OUT_DATA_1: begin
                    rdata <= int_conv_out[63:32];
                end
                ADDR_MAXPOOL_OUT_DATA_0: begin
                    rdata <= int_maxpool_out[31:0];
                end
                ADDR_MAXPOOL_OUT_DATA_1: begin
                    rdata <= int_maxpool_out[63:32];
                end
            endcase
        end
    end
...
assign conv_out    = int_conv_out;
assign maxpool_out = int_maxpool_out;
// int_img_in[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_img_in[31:0] <= 0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_CONV_OUT_DATA_0)
            int_conv_out[31:0] <= (WDATA[31:0] & wmask) | (int_conv_out[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_conv_out[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CONV_OUT_DATA_1)
            int_conv_out[63:32] <= (WDATA[31:0] & wmask) | (int_conv_out[63:32] & ~wmask);
    end
end

// int_maxpool_out[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_maxpool_out[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_MAXPOOL_OUT_DATA_0)
            int_maxpool_out[31:0] <= (WDATA[31:0] & wmask) | (int_maxpool_out[31:0] & ~wmask);
    end
end

// int_maxpool_out[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_maxpool_out[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_MAXPOOL_OUT_DATA_1)
            int_maxpool_out[63:32] <= (WDATA[31:0] & wmask) | (int_maxpool_out[63:32] & ~wmask);
    end
end
